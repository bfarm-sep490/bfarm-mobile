...
import {
  getDetailPackagingTasksById,
  putReportPackagingTasksById,
  uploadPackagingTaskImage,
} from '@/services/packaging-tasks';
...
export const PackagingDetail = () => {
  ...
  const id = Array.isArray(searchParams.id)
    ? searchParams.id[0]
    : searchParams.id;
  const [data, setData] = useState<IPackagingTask | null>(null);
  useEffect(() => {
    const fetchData = async () => {
      const { data, message, status } = await getDetailPackagingTasksById(
            ...
            >
              ...
              <>
                ...
                <Text className='line-clamp-2r'>
                  <Text className='font-bold'>Sản lượng đóng gói: </Text>{' '}
                  {data?.unpackaged_quantity} {data?.unpackaged_unit}
                </Text>

                <Text className='line-clamp-2r'>
                  <Text className='font-bold'>Sản phẩm đã đóng gói: </Text>{' '}
                  {data?.packaged_quantity} {data?.packaged_unit}
                </Text>
...
export const ReportPackagingTask = () => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<IReportPackagingTask>({
    defaultValues: {
      result_content: '',
      packaged_quantity: 0,
      packaged_unit: 'bao 20 kg',
      unpackaged_quantity: 0,
      unpackaged_unit: 'kg',
      list_of_images: [],
    },
  });
      ...
      async response => {
        if (response.errorMessage) {
          ...
          if (response.assets && response.assets.length > 0) {
            const { message, status, data } = await uploadPackagingTaskImage(
              response.assets,
            );
            if (status === 200 && data) {
  ...
  const onSubmit = async (report: IReportPackagingTask) => {
    const { status, data, message } = await putReportPackagingTasksById(1, {
      ...report,
                    ...
                    render={({ field: { onChange, onBlur, value } }) => (
                      <Input className='rounded'>
                        <InputField
                          ...
                          placeholder='Sản lượng đóng gói'
                        />
                      </Input>
                    )}
                    name='unpackaged_quantity'
                    rules={{
                      required: 'Tên vấn đề không được để trống',
                    }}
                  />
                  {errors.unpackaged_quantity && (
                    <Text className='text-red-500'>
                      {errors.unpackaged_quantity.message}
                    </Text>
                  )}
                </VStack>
                <VStack>
                  <Text className='ml-2 font-bold'>
                    Đơn vị sản lượng đóng gói
                  </Text>
                  ...
                  <Controller
                    ...
                    name='unpackaged_unit'
                    rules={{ required: 'Loại vấn đề không được để trống' }}
                  />
                  {errors.unpackaged_unit && (
                    <Text className='text-red-500'>
                      {errors.unpackaged_unit.message}
                    </Text>
                  )}
                </VStack>
                <VStack>
                  <Text className='ml-2 font-bold'>Sản phẩm đóng gói</Text>
                  <Controller
                    control={control}
                    render={({ field: { onChange, onBlur, value } }) => (
                      <Input className='rounded'>
                  ...
                  />
                  {errors.packaged_quantity && (
                    <Text className='text-red-500'>
                      {errors.packaged_quantity.message}
                    </Text>
                  )}
                </VStack>
                <VStack>
                  <Text className='ml-2 font-bold'>Đơn vị đóng gói</Text>
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, onBlur, value } }) => (
                      <RNPickerSelect
                        ...
                        onValueChange={onChange}
                        items={[
                          { label: 'bao 20kg', value: 'bao 20kg' },
                          { label: 'bao 50kg', value: 'bao 50kg' },
                        ]}
                      />
                    )}
                    name='unpackaged_unit'
